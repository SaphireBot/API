...
export function timeMs(Time: string | undefined): number | void {
...
export function getTimeout(TimeToCooldown = 0, DateNowInDatabase = 0, style: TimestampStylesString) {
    const Time = ~~((TimeToCooldown + DateNowInDatabase) / 1000)
    return time(Time, style as TimestampStylesString)
}